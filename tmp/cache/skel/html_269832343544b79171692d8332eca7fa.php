...
/*
 * Squelette : ../prive/themes/spip/box.css.html
 * Date :      Tue, 03 Dec 2024 10:08:50 GMT
 * Compile :   Mon, 27 Jan 2025 11:03:51 GMT
 * Boucles :   
 */ 
//
// Fonction principale du squelette ../prive/themes/spip/box.css.html
// Temps de compilation total: 0.772 ms
//

function html_269832343544b79171692d8332eca7fa($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
(($t1 = strval(retablir_echappements_modeles('')))!=='' ?
	...
	retablir_echappements_modeles(boite_ouvrir('', 'simple')) .
	' et cie.
...
': var(--spip-box-heading-iconpadding);
}
/* Bordure sur le côté */
.box.notice .box__header, .box.error .box__header, .box.success .box__header,
.box.notice .box__body, .box.error .box__body, .box.success .box__body,
.box.notice .box__footer, .box.error .box__footer, .box.success .box__footer {
	border-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['left'] ?? null))) .
': var(--spip-box-highlightsize) solid transparent;
}
...
.box.notice .box__body {
	border-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['left'] ?? null))) .
'-color: hsl(var(--spip-color-notice--h), var(--spip-color-notice--s), 60%);
}
.box.notice .box__footer {
	border-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['left'] ?? null))) .
'-color: hsl(var(--spip-color-notice--h), var(--spip-color-notice--s), 50%);
}
.box.notice .box__header,
.box.notice .box__body:first-child {
	background-image: url("' .
retablir_echappements_modeles(chemin_image((string)'warning-32.png')) .
'");
}
...
.box.error .box__body {
	border-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['left'] ?? null))) .
'-color: hsl(var(--spip-color-error--h), var(--spip-color-error--s), 60%);
}
.box.error .box__footer {
	border-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['left'] ?? null))) .
'-color: hsl(var(--spip-color-error--h), var(--spip-color-error--s), 50%);
}
.box.error .box__header,
.box.error .box__body:first-child {
	background-image: url("' .
retablir_echappements_modeles(chemin_image((string)'erreur-32.png')) .
'");
}
...
.box.success .box__body {
	border-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['left'] ?? null))) .
'-color: hsl(var(--spip-color-success--h), var(--spip-color-success--s), 55%);
}
.box.success .box__footer {
	border-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['left'] ?? null))) .
'-color: hsl(var(--spip-color-success--h), var(--spip-color-success--s), 48%);
}
.box.success .box__header,
.box.success .box__body:first-child {
	background-image: url("' .
retablir_echappements_modeles(chemin_image((string)'ok-32.png')) .
'");
}
...
.box.sous-rub .box__header .logo-carre {
	...
	margin-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['right'] ?? null))) .
': calc((var(--spip-box-spacing-x) / 2) * -1);
	margin-' .
retablir_echappements_modeles(interdire_scripts(($Pile[0]['left'] ?? null))) .
': auto;
	padding-' .
...
');
	...
	return analyse_resultat_skel('html_269832343544b79171692d8332eca7fa', $Cache, $page, '../prive/themes/spip/box.css.html');
}
